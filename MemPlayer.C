...
// printf("MemPlayer.C anfang Konstruktor: %p", *this);
  Initialized = FALSE;
  ServerAlive = FALSE;
  CoachActive = FALSE;
  ViewQuality = VQ_High;
...
// <COMMUNICATION>
	...
	LastPassTime = Time();
	PassTargetVector = Vector();
	Passpartner = 0;

	HeardPassTime = Time();
	HeardPassTargetVector = Vector();
	HeardPasspartner = 0;

// </COMMUNICATION>


// printf("MemAction.C ende Konstruktor: %p", *this);
}
...
/*********************************************************************************/
// <COMMUNICATION>
void PlayerInfo::HearPass(Unum pass_partner, Vector target, Time time)
{
	if(HeardPassValid(time))
	{
		if(pass_partner == MyNumber)
		{
			//// cerr << "PASS HEARD (" ;
			//// cerr << Mem->MyNumber << ", " << Mem->CurrentTime.t << "." << Mem->CurrentTime.s << "): ";
			//// cerr << time.t << "." << time.s << " (diff:" << (Mem->CurrentTime - time) << ") to ";
			//// if(pass_partner == MyNumber) cerr << " !!ME!! ";
			//// cerr << pass_partner << endl;

		}
		HeardPasspartner = pass_partner;
		HeardPassTime = time;
		HeardPassTargetVector = target;
		if(LastPassTime < time) SayPass(pass_partner, target, time);
	}

}

void PlayerInfo::SayPass(Unum pass_partner, Vector target, Time time)
{
	//// cerr << "PRIORITYPASSMESSAGE (" ;
	//// cerr << Mem->MyNumber << ", " << Mem->CurrentTime.t << "." << Mem->CurrentTime.s << "): ";
	//// cerr << time.t << "." << time.s << " (diff:" << (Mem->CurrentTime - time) << ") to ";
	//// cerr << pass_partner << endl;
		Passpartner = pass_partner;
		LastPassTime = time;
		PassTargetVector = target;
		Say.priority = 2;
}
bool   PlayerInfo::PlayedPass() 
{
	if (CurrentTime.t < 1) return false;
	return ((CurrentTime - LastPassTime) < OP_communicate_pass_valid_cycles); 
}

void   PlayerInfo::SetPassTime(Time pass_time) { LastPassTime = pass_time; }
void   PlayerInfo::SetPassTargetVector(Vector pass_target) { PassTargetVector = pass_target; }
void   PlayerInfo::SetPasspartner(Unum passpartner) { Passpartner = passpartner; }

bool   PlayerInfo::HeardPassValid() { return ((CurrentTime - GetHeardPassTime()) < OP_communicate_pass_valid_cycles); }
bool   PlayerInfo::HeardPassValid(Time time) { return ((CurrentTime - time) < OP_communicate_pass_valid_cycles); }

Time   PlayerInfo::GetHeardPassTime() { return HeardPassTime; }
Vector PlayerInfo::GetHeardPassTargetVector() { return HeardPassTargetVector; }
Unum   PlayerInfo::GetHeardPasspartner() { return HeardPasspartner; }
...
void PlayerInfo::SetMySensedInfo(float st, float e, float sp, float spa, float ha, int k, int d, int tu, int sa, int tn, Time ti)
{
  if ( sense_time == ti )
    return;
...
int PlayerInfo::MyCommunicationInterrupt()
{
	...
	{
		return Mem->CP_interrupts_per_cycle - 1; 
	}
/*
  int comInt;
  Time time = Mem->CurrentTime;
  for(int interrupt = 1; interrupt <= Mem->CP_interrupts_per_cycle; interrupt++) {
  comInt = ((time % 22) / 2 + interrupt + (time % 2) * 6) % 11 + 1;
  if (comInt == Mem->MyNumber) return comInt;
  }
  return 0;
*/
