...
int Net::create_layers(int layers, int layer_nodes[]){
  ...
  if(topo_data.unit_count){
      cerr<<"Can't create layers - network already defined\n";
    return(ERROR);
  }
...
void Net::create_units()
{
  ...
  if(unit == NULL){
      cerr<<"Error: Can't allocate units\n";
    exit(0);
  }
  /* create bias unit */
  unit[0].unit_no = 0;    
...
void Net::connect_units(int to_unit, int from_unit, FTYPE value){
  ...
  if(! IS_UNIT(to_unit) || (!IS_UNIT(from_unit)&& from_unit!=BIAS)  ){
      cerr<<"connect_units: not possible from "<<from_unit<<" to "<<to_unit<<endl;
    return;
  }
  ...
  if(unit[to_unit].weights==NULL){/* weight list empty */ 
    ...
    for(wptr=unit[to_unit].weights; wptr->next!=NULL;wptr=wptr->next){
	  ...
	  cerr<<"Connection "<<from_unit<<"->" <<to_unit<<" already exists\n";
	/* destroy(temp) */
	return;
      }
    } /* end of list reached */
...
int Net::save_net( char filename[] )
{
  ...
  if (outfile==NULL){
      cerr<<"Output file "<<filename<<" cannot be opened\n";
    return(FILE_ERROR);
  }
...
int Net::load_net( char filename[] )
{
  ...
  if (infile==NULL){
	...
	cerr<<"Neither "<<filename<<" nor "<<secondchance<<" can be opened\n";
    return(FILE_ERROR);
    }
  }
  ...
  while(fgets(line,MAX_LINE_LEN,infile)!=NULL){
	...
	if ((position<1) || (position > topo_data.in_count)){
	    cerr<<"Error: Input_scale - undefined position "<<position<<"\n";
	  return(ERROR);
	}
	...
	if ((position<1) || (position > topo_data.out_count)){
	    cerr<<"Error: Output_scale - undefined position "<<position<<"\n";
	  return(ERROR);
	}
...
/* set activation function of unit unit_no */
  if(!IS_UNIT(unit_no)){
      cerr<<"set act_f: Unit "<<unit_no<<" doesn't exists\n";
    return;
  }
  if(IS_INPUT(unit_no)){
      cerr<<"set act_f: No sense to set activation function of input unit "<<unit_no<<endl;
    return;
  }
  switch(act_id){
  case LOGISTIC:
  ...
  case LINEAR:
    ...
    break;
  default:
      cerr<<"set act_f: Unknown activation function "<<act_id<<endl;
  }
}
...
/* set activation function of a complete layer */
  ...
  if(!IS_LAYER(layer_no)){
      cerr<<"set_layer_act_f: Layer "<<layer_no<<"doesn't exist\n";
    return;
  }
...
/* set current update function, init update_function, set update_params */
{
  ...
  case RPROP:
    ...
    break;
  default:
      cerr<<"set update_f: Unknown update function "<<typ<<endl;
    return (ERROR);
  }
