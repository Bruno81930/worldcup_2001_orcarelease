...
void PositionInfo::Initialize()
{
  ...
  if ( MySide == 'l' ){
    ...
    RM_My_Goal_T= Flag_GLT;
    RM_My_Goal_B= Flag_GLB;
    RM_Their_Goal_T=Flag_GRT;
    RM_Their_Goal_B=Flag_GRB;
    RM_LB_Flag = Flag_LT;
    RM_LC_Flag = Flag_CT;
    RM_LF_Flag = Flag_RT;
    RM_LF20_Flag = Flag_RT20;   
    RM_LF10_Flag = Flag_RT10;   
    RM_RB_Flag = Flag_LB;
    RM_RC_Flag = Flag_CB;
    RM_RF_Flag = Flag_RB;
    RM_RF10_Flag = Flag_RB10;   
    RM_RF20_Flag = Flag_RB20;   
    RM_My_PC_Flag = Flag_PLC;    /* Center of my penalty area */
    RM_Their_PC_Flag = Flag_PRC; /* Center of theirs          */
    RM_Flag_T50= Flag_TL50;
    RM_Flag_B50= Flag_BL50;
  }
  else{ /* MySide == 'r' */
    RM_My_Goal = Goal_R;
    ...
    RM_LC_Flag = Flag_CB;
    RM_LF_Flag = Flag_LB;
    RM_LF20_Flag = Flag_LB20;    
    RM_LF10_Flag = Flag_LB10;    
    RM_RB_Flag = Flag_RT;
    RM_RC_Flag = Flag_CT;
    RM_RF20_Flag = Flag_LT20;
    RM_RF10_Flag = Flag_LT10;
    RM_RF_Flag = Flag_LT;
    RM_My_PC_Flag = Flag_PRC;    /* Center of my penalty area */
    RM_Their_PC_Flag = Flag_PLC; /* Center of theirs          */
    RM_Flag_T50= Flag_TR50;
    RM_Flag_B50= Flag_BR50;
    RM_My_Goal_T= Flag_GLT;
    RM_My_Goal_B= Flag_GLB;
    RM_Their_Goal_T=Flag_GRT;
    RM_Their_Goal_B=Flag_GRB;
...
/// Strategy *****************************
int PositionInfo::GetOffense()
{
	if (CurrentTime > offense_last_updated_time)
	{
		if (TeamWithBall() == MySide) 
		{
			offense = 1;
			offense_time = CurrentTime;
			offense_dist = BallDistance();
		}
		
		if (TeamWithBall() == TheirSide) { 
			offense = 0; 
			offense_time = CurrentTime; 
			offense_dist = BallDistance();
		}
		
		offense_last_updated_time = CurrentTime;
		//cerr << "offense: ";
		//cerr << MyNumber << " (" << CurrentTime.t << "." << CurrentTime.s << ") ";
	//	if (offense)
	//	{
	//		cerr << "\t\tOFFENSE (" << offense_time.t << "." << offense_time.s << ")";
	//		cerr << " dist: " << offense_dist << "m";
	//	}
	//	else 
	//	{
	//		cerr << "\t\t\tDEFENSE (" << offense_time.t << "." << offense_time.s << ")";
	//		cerr << " dist: " << offense_dist << "m";
	//	}	
	//	cerr << endl;
	}
...
float PositionInfo::GetOffenseConf()
{
	// not yet implemented
	return 0;
}

Time PositionInfo::GetOffenseTime()
{
	return offense_time;
}

float PositionInfo::GetOffenseDist()
{
	return offense_dist;
}


void PositionInfo::SetOffense(int offense_value, Time offenseTime, float offenseDist, Unum sender, Time time)
{
	if((offenseTime > offense_time)
	   || ((offenseTime >= offense_time)
		   && (offenseDist < offense_dist)))
	{
		offense = offense_value;
		offense_time = offenseTime;
		//offense_last_updated_time = CurrentTime;
		offense_dist = offenseDist;
	}
}

/// Opponents **********************
int PositionInfo::GetPlayerType(char s, Unum n)
{
	// not yet implemented
	return 0;
}

float PositionInfo::GetPlayerTypeConf(char s, Unum n)
{
	// not yet implemented
	return 0;
}

void PositionInfo::SetPlayerType(int pType, float conf, char s, Unum n)
{
	// not yet implemented
	;
}
...
/// Mates *********************************
float PositionInfo::GetStamina(char s, Unum n)
{
	PlayerObject *player;
	if ( (player = GetPlayer(s, n)) == NULL ) return 0;
	return player->get_stamina();
}

float PositionInfo::GetStaminaConf(char s, Unum n)
{
	PlayerObject *player;
	if ( (player = GetPlayer(s, n)) == NULL ) return 0;
	return player->get_stamina_conf();
}

void PositionInfo::SetStamina(int stamina, float conf, float dist, Time time, char s, Unum n)
{
	PlayerObject *player;
	if ( (player = GetPlayer(s, n)) == NULL ) return;
	if (conf > player->get_stamina_conf())
	{
		player->set_stamina(stamina);
		player->set_stamina_conf(conf);
		player->set_stamina_dist(dist);
		player->set_stamina_time(time);
	}
}
...
void PositionInfo::HearPlayer(char side, Unum num, float x, float y, float conf, float dist, Time time)
{
  /* When hearing a player location, remember not to call just getplayer, but also
    ...
    my_error("Hearing Player %c %d (%.1f %.1f)",side,num,x,y);
			      ...
			      float dx, float dy, float vconf, float dist, Time time)
{
  ...
  if ( (player = GetPlayer(side, num)) == NULL ){
	  Vector pos = Vector(x,y);
	  if ( (player = ClosestPlayerObjectTo(side, pos)) != NULL ){
		  if ( player->side == '?' ) player->side = side; 
		  player->unum  = num;
	  }
	  else player = GetNewPlayer(side, num);
  }
...
Bool PositionInfo::BallKickableForPlayer(char s, Unum n, float buffer=0)
{
  ...
  return ((pos - BallAbsolutePosition()).mod() <
    SP_kickable_area - buffer) ? TRUE : FALSE;
}
...
// <timo>
Vector PositionInfo::coachPositionToPullOffsidePosition(Vector pos, float buffer)
{
  if ( !SP_use_offside ) return pos;

  float pull_x = coachPullOffsidePosition+buffer;	// the only change actually :-)
  if ( BallPositionValid() )
    pull_x = Min(pull_x, BallX());  
