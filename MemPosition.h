...
private:
  ...
  float    hstamina;      /* heard stamina (mates only) */
  float    hstaminaconf;
  float    hstaminadist;
  Time     hstaminatime;
  
  int      ptype;         /* heard Playertype */
  float    ptypeconf;
  Time     ptypetime;
  
  int      hptype;        /* heard Playertype */
  float    hptypeconf;
  float    hptypedist;
  Time     hptypetime;
  
public:
  void set_hptype(int ptype) { hptype = ptype; };
  void set_hptype_dist(float dist) { hptypedist = dist; }
  void set_hptype_conf(float conf) { hptypeconf = conf; }
  void set_hptype_time(Time time) { hptypetime = time; }
  
  void set_ptype(int ptype) { ptype = ptype; }
  void set_ptype_conf(float conf) { ptypeconf = conf; }
  void set_ptype_time(Time time) { ptypetime = time; }
  
  void set_stamina(float stamina) { hstamina = stamina; }
  void set_stamina_dist(float dist) { hstaminadist = dist; }
  void set_stamina_conf(float conf) { hstaminaconf = conf; }
  void set_stamina_time(Time time) { hstaminatime = time; }
  
  float get_hptype() { return hptype; }
  float get_hptype_dist() { return hptypedist; }
  float get_hptype_conf() { return hptypeconf; }
  Time get_hptype_time() { return hptypetime; }
  
  float get_ptype() { return ptype; }
  float get_ptype_conf() { return ptypeconf; }
  Time get_ptype_time() { return ptypetime; }
  
  float get_stamina() { return hstamina; }
  float get_stamina_dist() { return hstaminadist; }
  float get_stamina_conf() { return hstaminaconf; }
  Time get_stamina_time() { return hstaminatime; }
...
*****************************************************************************************/
	...
	Time GetOffenseTime();
	float GetOffenseDist();
	void SetOffense(int offense, Time otime, float dist, Unum sender, Time time);
	
	/// Opponents **********************
	int GetPlayerType(char s, Unum n);
	float GetPlayerTypeConf(char s, Unum n);
	void SetPlayerType(int pType, float conf, char s, Unum n);
	
	/// Mates *********************************
	float GetStamina(char s, Unum n);
	float GetStaminaConf(char s, Unum n);
	void SetStamina(int stamina, float conf, float dist, Time time,  char s, Unum n);

/****************************************************************************************/
/* ENDE VON ORCA COMMUNICATION STUFF 
*****************************************************************************************/
  ...
  MarkerType RM_LF20_Flag;
  MarkerType RM_LF10_Flag;
  MarkerType RM_RB_Flag;
  MarkerType RM_Flag_T50;
  MarkerType RM_Flag_B50;
  MarkerType RM_Their_Goal_T;
  MarkerType RM_Their_Goal_B; 
  MarkerType RM_My_Goal_T;
  MarkerType RM_My_Goal_B;
  MarkerType RM_RC_Flag;
  MarkerType RM_RF_Flag;
  MarkerType RM_RF20_Flag;
  MarkerType RM_RF10_Flag;
  MarkerType RM_My_PC_Flag;    /* Center of my penalty area */
  MarkerType RM_Their_PC_Flag;
  ...
  // <timo> equivalent function to above, yet coachPullOffsidePosition is used
  Vector coachPositionToPullOffsidePosition(Vector pos, float buffer);
  inline Vector coachPositionToPullOffsidePosition(Vector pos) 
    { return coachPositionToPullOffsidePosition(pos,CP_at_point_buffer); }
  // </timo>
  ...
  // <timo>
  float coachPullOffsidePosition;	// general tip to do an offside trap
  // </timo>
...
private:
  ...
  //<COMMUNICATION>
	Time offense_last_updated_time;
	Time offense_time;
	float offense_dist;
	int  offense;
